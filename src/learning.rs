...
impl ProjectDna {
    ...
    fn detect_conventions(&mut self, cwd: &Path) {
        ...
        if cwd.join(".eslintrc.json").exists() || cwd.join(".eslintrc.js").exists() {
            self.conventions
                .push("ESLint configured — respect rules".into());
        }
        if cwd.join(".prettierrc").exists() {
            self.conventions
                .push("Prettier configured — match formatting".into());
        }
        if cwd.join("rustfmt.toml").exists() {
            self.conventions
                .push("rustfmt configured — match formatting".into());
        }
            ...
            if let Ok(content) = std::fs::read_to_string(cwd.join(".editorconfig")) {
                for line in content.lines() {
                    if line.contains("indent_style") {
                        self.conventions
                            .push(format!("EditorConfig: {}", line.trim()));
                    }
                }
            }
        }
...
pub fn record_learning(error_output: &str, tool_name: &str, success_after: bool) {
    ...
    if pattern.is_empty() || lesson.is_empty() {
        return;
    }
            ...
            if i == line && l.contains("- [") {
                if let Some(bracket_end) = l.find(']') {
                    ...
                    if let Ok(count) = count_str.parse::<u32>() {
                        let new_line =
                            l.replace(&format!("[{}]", count), &format!("[{}]", count + 1));
...
fn analyze_error(error: &str, tool_name: &str) -> (String, String, String) {
    ...
    if lower.contains("no such file") || lower.contains("not found") {
        return (
            "file not found".into(),
            format!(
                "Verify file paths before {}. Use list_files or glob to confirm paths exist.",
                tool_name
            ),
            "filesystem".into(),
        );
    ...
    if error.len() > 50 {
        ...
        return (
            pattern,
            format!(
                "Error pattern detected during {} — investigate and fix root cause",
                tool_name
            ),
            "general".into(),
        );
