...
fn analyze_dockerfile(path: &Path, content: &str) -> Vec<Finding> {
    ...
    for (i, line) in content.lines().enumerate() {
        ...
        if l.to_lowercase().starts_with("from ") && (l.contains(":latest") || !l.contains(':')) {
            findings.push(Finding {
                ...
                message: format!(
                    "line {}: FROM uses `:latest` or no tag — pin a version",
                    i + 1
                ),
            });
        }
        if l.to_lowercase().starts_with("user root") {
            findings.push(Finding {
                ...
                message: format!(
                    "line {}: container runs as root — add a non-root USER",
                    i + 1
                ),
            });
        }
    ...
    for (i, line) in content.lines().enumerate() {
                ...
                .any(|k| l.contains(k))
        {
            findings.push(Finding {
                ...
                severity: Severity::High,
                message: format!(
                    "line {}: secret-like value in ENV/ARG — use build secrets or runtime env",
                    i + 1
                ),
            });
        }
    ...
    if lower.matches("from ").count() == 1 && content.len() > 2_000 {
        findings.push(Finding {
            ...
            message: "single-stage Dockerfile — consider multi-stage build for smaller images"
                .into(),
        });
    }
...
pub fn security_scan() {
    ...
    if files.is_empty() {
        println!(
            "\n  {} No infrastructure files to scan.",
            "[INFRA]".dimmed()
        );
        return;
    }
...
mod tests {
    ...
    fn dockerfile_latest_tag_flagged() {
        let f = analyze_dockerfile(
            &PathBuf::from("Dockerfile"),
            "FROM alpine:latest\nCMD [\"sh\"]\n",
        );
        assert!(f
            .iter()
            .any(|x| x.severity == Severity::Warn && x.message.contains("latest")));
    }
    ...
    fn terraform_open_ingress_flagged() {
        let tf =
            "resource \"aws_security_group\" \"x\" { ingress { cidr_blocks = [\"0.0.0.0/0\"] } }";
